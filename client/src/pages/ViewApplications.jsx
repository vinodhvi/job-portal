    ...
    <div className='container mx-auto p-4'>
      <div>
        <table className='w-full max-w-4xl bg-white border border-gray-200 max-sm:text-sm'>
          ...
          <tbody>
            {viewApplicationsPageData.map((viewApp, index) => (
              <tr key={index} className='text-gray-700'>
                  <td className='py-2 px-4 border-b text-center'>{index + 1}</td>
                  <td className='py-2 px-4 border-b text-center flex items-center'>
                    <img className='w-10 h-10 rounded-full mr-3 max-sm:hidden' src={viewApp.imgSrc} alt='img' />
                    <span>{viewApp.name}</span>
                  </td>
                  <td className='py-2 px-4 border-b max-sm:hidden'>{viewApp.jobTitle}</td>
                  <td className='py-2 px-4 border-b max-sm:hidden'>{viewApp.location}</td>
                  <td className='py-2 px-4 border-b'>
                    <a href=''
                    className='bg-blue-50 text-blue-400 px-3 py-1 rounded inline-flex gap-2 items-center'
                     target='_blank'>
                      Resume <img src={assets.resume_download_icon} alt='resumer' />
                    </a></td>
